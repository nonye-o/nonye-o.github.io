...
num_updates = 20000         # Total number of parameter updates 
...
for i in range(num_updates):
    ...
    for j in range(num_steps_to_update):
        ...
        observation_array = np.vstack((observation_array, observations.reshape(1, *observations.shape)))
    print(f'Time taken to get transitions: {time.time() - start_time}')
    ...
    for k in range(num_workers):
        ...
        print(f'Time taken to calculate targets and loss for worker {k}: {time.time()-start_time}')
    ...
    loss.backward()
    optimizer.step()
    print(f'Time taken to update models: {time.time()-start_time}')    

    # Save the models at some milestones
    if i%1000 == 0:
        print(f'Update {i}/{num_updates}\nloss: {loss}\n-----------------')
        torch.save({
